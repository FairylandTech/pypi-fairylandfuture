...
class HTTPSimpleRequest:
    ...
    def __init__(self, headers: dict[str, str] | None = None, cookies: dict[str, str] | None = None, verify: bool = False, timeout: int | None = None):
        self.headers = self._make_headers(headers)
        self.cookies = cookies if cookies else {}
        self.verify = verify
        self.timeout = timeout if timeout else 30

    def _make_headers(self, headers: dict[str, str] | None) -> dict[str, str]:
        if not headers:
    ...
    def get(self, url: str, params: dict[str, Any] | None = None) -> HTTPSimpleRequestResultStructure:
        try:
    ...
    def post(self, url: str, data: dict[str, Any] | None = None) -> HTTPSimpleRequestResultStructure:
        try:
